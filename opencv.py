...
def detect_and_predict(frame):
    ...
    scale_factor = min(1, 640 / max(height, width))  # Limit max dimension to 640px
    if scale_factor < 1:
        frame = cv2.resize(frame, None, fx=scale_factor, fy=scale_factor)
        height, width = frame.shape[:2]

    window_size = (224, 224)
    stride = 112  # Adjusted stride for better coverage
    ...
    boxes, final_scores = non_max_suppression(detections, scores, 0.3)  # Adjusted overlap threshold
...
while True:
    ...
    # Display the processed frame if available, otherwise display the original frame
    try:
        display_frame = result_queue.get_nowait()
        cv2.imshow('Fruit and Vegetable Quality Detection', display_frame)
    except queue.Empty:
        cv2.imshow('Fruit and Vegetable Quality Detection', frame)
